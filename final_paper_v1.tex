...
% =========================================================================
\begin{abstract}
Graph representation learning has become an important paradigm for analyzing
networked data across domains such as social networks, citation graphs, and
biological systems. Many scalable graph embedding approaches rely on dense
Gaussian random projections, which can impose significant computational and
memory overhead when applied to large graphs. This work investigates whether
hybrid structured random projections can improve the trade-off between
efficiency and predictive performance within the FastRP graph embedding
framework.

Three projection strategies are designed and compared: (i) a standard
Gaussian projection as the baseline, (ii) a striped structured sparse
projection intended to reduce density while preserving embedding quality, and
(iii) a hybrid projection that concatenates a structural embedding component
with a node feature component. Experiments are conducted on four benchmark
datasets---BlogCatalog, Flickr, Cora, and ogbn-arxiv---covering node
classification tasks, and on Cora, Citeseer, and PubMed for link prediction.
Hyperparameters are optimized using Optuna with 50 trials per configuration,
and downstream performance is evaluated using Macro-F1 score, classification
accuracy, area under the ROC curve (AUC), and mean reciprocal rank (MRR@10).

On the Cora citation network, the hybrid projection achieves a Macro-F1 of
0.7541, outperforming the Gaussian baseline (0.7232) and the striped
projection (0.7356). On the large-scale ogbn-arxiv benchmark, the hybrid
approach yields a test accuracy of 0.6891 versus 0.5211 for the Gaussian
baseline---a gain of 16.8 percentage points---demonstrating that integrating
node feature information into the projection substantially improves embedding
quality on feature-rich graphs. The striped structured projection is
competitive on citation graphs but shows sensitivity to dataset
characteristics on Flickr, where it trails the Gaussian baseline by a small
margin. Overall, the results indicate that carefully designed hybrid
structured projections are a promising direction for scalable graph
...
for many PubMed query nodes. One plausible explanation is that PubMed's
feature space (TF-IDF over biomedical vocabulary) is high-dimensional and
sparse, so the feature projection---which uses a Gaussian matrix scaled by
$1/F$---may require more than $d/4$ dimensions to faithfully represent
biomedical relatedness, leading to imprecise top-$k$ ranking even when
global AUC is better. This interpretation motivates future experiments that
vary the structure-to-feature split ratio beyond the fixed 3:1 used here.

\subsubsection{Computational Efficiency}

A key design goal of projection-based embeddings is to eliminate the
backpropagation cost of end-to-end GNNs. All three methods in this study
...
CPU times reported in Table~\ref{tab:lp_time} confirm that all variants
finish in well under two seconds even on PubMed (19.7K nodes).
...
strategies were compared---a dense Gaussian baseline, a striped structured
sparse projection, and a hybrid structure-feature projection---across node
classification and link prediction tasks on seven benchmark datasets.
